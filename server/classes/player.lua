function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried setting -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.setBankBalance = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried setting -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried adding -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.removeMoney = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried removing -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.addBank = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried adding -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.removeBank = function(money)
		...
		if money >= 0 then
			...
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried removing -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.getMissingAccounts = function(cb)
		local params = {collection = "user_accounts",query = {identifier = self.getIdentifier()},options = {projection = {name = 1, money = 1, _id = 0}}}
		exports.DatabaseAPI:find(params, function (success, result)
			if not success then
				print("[MongoDB] [ESBridge] ERRO AO OBTER A DATA DAS CONTAS DO USUARIO: "..tostring(result))
				return
			end
	...
	self.createAccounts = function(missingAccounts, cb)
		for k,v in ipairs(missingAccounts) do
			exports.DatabaseAPI:insertOne({ collection="user_accounts", document = {identifier = self.getIdentifier(), name = v, money = 0} }, function (success, result, insertedIds)
				if not success then
					print("[MongoDB] [ESBridge] ERRO AO CRIAR CONTAS: "..tostring(result))
					return
				end
				print("[MongoDB] [ESBridge] CONTAS DE USUARIO CRIADA. ID: "..tostring(insertedIds[1]))
				if cb then
					cb()
				end
			end)
	...
	self.setAccountMoney = function(acc, money)
		if money < 0 then
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried setting -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.addAccountMoney = function(acc, money)
		if money < 0 then
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried adding -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.removeAccountMoney = function(acc, money)
		if money < 0 then
			print(('[CBR Framework] [ALERTA]: %s attempted exploiting! (reason: player tried removing -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.setJob = function(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('[CBR Framework] [ALERTA]: PULANDO SETAGEM DO JOB: %s DEVIDO NÃO EXISTIR!'):format(self.source))
		end
	end
