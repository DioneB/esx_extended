AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			exports.DatabaseAPI:updateOne({ collection="users", query = { identifier = player.getIdentifier()}, update = { ["$set"] = {name = userData.playerName} } }, function (success, users)
				if not success then
					print("[MongoDB] [ESBridge] ERRO AO ATUALIZAR NOME DO USUARIO: "..tostring(users))
					return
				end	
				cb()		
			end)			
		...
		-- Get accounts
		table.insert(tasks, function(cb)
			local params = {collection = "user_accounts",query = {identifier = player.getIdentifier()},options = {projection = {_id = 0}}}
			exports.DatabaseAPI:find(params, function (success, accounts)
				if not success then
						print("[MongoDB] [ESBridge] ERRO AO OBTER AS CONTAS DO USUARIO")
					return
				end
		...
		-- Get inventory
		table.insert(tasks, function(cb)
			local params = {collection = "user_inventory",query = {identifier = player.getIdentifier()},options = {projection = {item = 1, count = 1, _id = 0}}}
			exports.DatabaseAPI:find(params, function (success, inventory)
				if not success then
						print("[MongoDB] [ESBridge] ERRO AO OBTER AS CONTAS DO USUARIO")
					return
				end
				...
				for k,v in ipairs(inventory) do
					...
					if item then
						...
						print(('[CBR Framework] [ALERTA]: O ITEM: "%s" É INVALIDO! IGNORANDO'):format(v.item))
					end
				end
					...
					if not found then
						...
						local scope = function(item, identifier)
							table.insert(tasks2, function(cb2)
								...
								exports.DatabaseAPI:insertOne({ collection="user_inventory", document = IData }, function (success, result, insertedIds)
									if not success then
										print("[MongoDB] [ESBridge] ERRO AO INSERIR ITEM NO INV: "..tostring(result))
										return
									end
									-- print("[MongoDB] [ESBridge] ITEM INSERIDO. ID UNICA: "..tostring(insertedIds[1]))
									cb2()
								end)
							end)
						end
		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
				local params2 = {collection = "users",query = {identifier = player.getIdentifier()},options = {projection = {job = 1, job_grade = 1, loadout = 1, position = 1, _id = 0}}}
				exports.DatabaseAPI:find(params2, function (success, result)
					if not success then
							print("[MongoDB] [ESBridge] ERRO AO OBTER AS INFOS DE TRABALHO E LOADOUT DO USUARIO")
						return
					end
					...
					if ESX.DoesJobExist(job, grade) then
						...
						print(('[CBR Framework] [ALERTA]: %s NÃO POSSUI UM JOB [SET: %s, CARGO: %s], DEFININDO COMO DESEMPREGADO (MEDIDA PADRÃO)!'):format(player.getIdentifier(), job, grade))
